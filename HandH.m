...
part = '5';         % Options are '2', '3', '4', '5', '6' for question part
...
% Replace the switch case block with this if-else structure
...
elseif part == '6'
    ...
    fprintf('Selection must be from 1 to 6.')
end
...
for pulse = 1:Npulses    % For each current pulse
    ...
    % make applied current a new value for the duration of current pulse
    for i = round(pulsestart / dt) + 1:round(pulsestop / dt)
        Iapp(i) = Ie;
    end
end

% Initialize all variables in the simulation

V = zeros(size(t));      % voltage vector
V(1) = V0;               % set the initial value of voltage
...
h = zeros(size(t));      % h: sodium inactivation gating variable
...
for i = 2:length(t)  % now see how things change through time
    ...
    I_Na(i) = G_Na * m(i)^3 * h(i) * (E_Na - V(i-1));  % total sodium current
    I_K(i) = G_K * n(i)^4 * (E_K - V(i-1));  % total potassium current
